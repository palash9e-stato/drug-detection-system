...
import graphviz
from tamper import store_evidence, verify_chain, get_evidence_count, publish_to_chain, generate_zk_proof, mint_sbt, SmartContract, load_chain, save_chain
...
if "behavior_engine" not in st.session_state:
    st.session_state.behavior_engine = BehaviorAnalyzer()

if "scene_reconstructor" not in st.session_state:
    st.session_state.scene_reconstructor = SceneReconstructor()
...
with st.sidebar:
    ...
    offline_mode = st.toggle("Offline Mode", value=False, help="Disable external API calls (Gemini, Maps, Location) for isolated environments.")
    st.session_state['offline_mode'] = offline_mode
    
    if offline_mode:
        st.warning("⚠️ OFFLINE: Cloud Analysis & Maps Disabled")
    ...
    with col_left:
        with st.container(height=500, border=True):
            ...
            if locations:
                ...
                if st.session_state.get('offline_mode', False):
                    st.warning("Maps Disabled in Offline Mode")
                    # Simple grid or empty container
                    st.markdown("""
                    <div style="height:400px; background:#111; display:flex; align-items:center; justify-content:center; color:#555;">
                        OFFLINE MAP VIEW <br> (Low Bandwidth Grid)
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    layer = pdk.Layer(
                ...
                st.pydeck_chart(pdk.Deck(
                    ...
                    layers=[layer],
                    tooltip={"html": "<b>{id}</b> <br> Status: {status} <br> Confidence: {confidence:.2f}", "style": {"color": "white"}}
                ))
                ...
                with c2:
                    with st.spinner("Multi-modal analysis..."):
                        ...
                        if res and res[0].boxes and len(res[0].boxes) > 0:
                            ...
                            st.image(cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB), caption="Source", use_container_width=True)
                            st.info("No objects detected by YOLO.")
                        
                        # Gemini Vision
                        st.write("Generating Semantic Analysis...")
                        try:
                            prompt = "Analyze image for illegal drugs/contraband. Check text against controlled substance lists. If standard medicine (e.g. Ketoconazole), return is_threat=False, description='LEGAL: Common Pharmaceutical'. If banned/suspicious, return is_threat=True. Description MUST be <10 words. Return JSON: {'is_threat': bool, 'description': str}"
...
def render_webcam():
    ...
    # Psycho-Pass Module
    use_psych_profiling = st.toggle("🧠 Behavioral Profiling (Psycho-Pass)", value=False)
    if use_psych_profiling:
        st.caption("Analyzing micro-expressions and body language cues...")
        col_m1, col_m2 = st.columns(2)
        with col_m1: stress_metric = st.empty()
        with col_m2: status_metric = st.empty()
    ...
    if active and yolo_model:
        ...
        while active:
            ...
            # 3. PSYCHO-PASS BEHAVIORAL ANALYSIS
            if use_psych_profiling:
                # Run Analysis
                display_frame, metrics = st.session_state.behavior_engine.analyze_frame(display_frame)
                
                # Update UI Metrics
                s_score = metrics["stress_level"]
                s_color = "normal"
                if s_score > 0.6: s_color = "off" # Red/Inverse logic in diff themes, stick to value
                
                stress_metric.metric("Psych Stress", f"{s_score:.2f}", delta="High" if s_score > 0.6 else "Normal", delta_color="inverse")
                status_metric.metric("Subject Status", metrics["status"])
                
                if metrics["surrender_detected"]:
                    st.toast("🏳️ SURRENDER DETECTED - HOLD FIRE")
                    
                if metrics["aggression_detected"] or s_score > 0.8:
                    log_ph.error(f"⚠️ HIGH THREAT BEHAVIOR: {metrics['status']}")
                    # Store Evidence
                    if np.random.rand() > 0.95:
                         store_evidence({
                            ...
                            "confidence": s_score,
                            "meta": {"location": get_location_data()}
                        })
...
with st.sidebar:
    st.title("🛡️ GUARDIAN")
    menu = st.radio("MODULES", ["Dashboard", "Social Network Graph", "Semantic Intelligence", "Visual Forensics", "3D Forensics", "Whistleblower Portal", "Video Forensics", "Audio Intelligence", "Live Surveillance"])
    st.markdown("---")
...
def render_3d_module():
    st.title("✨ 3D Scene Reconstruction (Minority Report)")
    st.caption("Convert 2D surveillance snapshots into navigable 3D Point Clouds.")
    ...
    if uploaded_file:
        ...
        st.image(image, caption="Source 2D Image", use_container_width=True)
        
        if st.button("GENERATE 3D HOLOGRAM", type="primary"):
            with st.spinner("Calculating Depth Map & Projecting Points..."):
                fig = st.session_state.scene_reconstructor.process_image(image)
                st.plotly_chart(fig, use_container_width=True)
                st.success("Hologram Generated.")
                st.info("Use mouse to Rotate, Zoom, and Pan the 3D scene.")

def render_whistleblower_portal():
    st.title("🕵️ Designated Whistleblower Portal (Tor-Bridge)")
    st.caption("Submit evidence anonymously. Zero-Knowledge Proofs ensure payload validity without revealing source.")
    
    with st.expander("How this works", expanded=False):
        st.markdown("""
        1. **Upload Evidence**: File is hashed locally.
        2. **Generate Proof**: ZK-Circuit proves file contains relevant metadata/structure.
        3. **Submit**: Only the Proof and Hash are sent on-chain.
        4. **Reward**: Smart Contract releases bounty to your generated keypair.
        """)
    
    c1, c2 = st.columns(2)
    with c1:
        st.subheader("1. Secure Upload")
        w_file = st.file_uploader("Evidence File", help="Metadata scrubbed automatically.")
        
    with c2:
        st.subheader("2. Cryptographic Status")
        if w_file:
            st.info("File Analysis: INTEGRITY CONFIRMED")
            if st.button("GENERATE ZK-PROOF", type="primary"):
                with st.spinner("Computing ZK-SNARK (Groth16)..."):
                    time.sleep(2) # Sim
                    # Mock proof
                    proof = f"zk-snark-proof:0x{hash(w_file.name)}..."
                    st.session_state.current_proof = proof
                    st.success("Proof Generated!")
                    st.code(proof, language="text")
        
    if st.session_state.get("current_proof"):
        st.markdown("---")
        st.subheader("3. On-Chain Submission")
        wallet = st.text_input("Payout Wallet (ETH/SOL)", "0x...")
        if st.button("SUBMIT TO DAO"):
            st.balloons()
            st.success(f"Transaction Confirmed: 0x{int(time.time())}a2f")
            mint_sbt("ANON_WHISTLEBLOWER", "SUBMISSION_VERIFIED", "PROOF-123")
...
elif menu == "3D Forensics":
    render_3d_module()
elif menu == "Whistleblower Portal":
    render_whistleblower_portal()
