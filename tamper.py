...
EVIDENCE_FILE_DEFAULT = "evidence_chain.json"
SESSION_POINTER_FILE = ".current_session_evidence"

def init_new_session():
    """Initializes a new evidence chain file for the current session."""
    filename = f"{int(time.time())}_evidence_chain.json"
    with open(SESSION_POINTER_FILE, 'w') as f:
        f.write(filename)
    
    # Init empty chain
    save_chain({"chain": [], "last_hash": "0"}, filename=filename)
    print(f"[TAMPER] New Session Initialized: {filename}")
    return filename

def get_current_filename():
    """Returns the current session's evidence filename."""
    if Path(SESSION_POINTER_FILE).exists():
        with open(SESSION_POINTER_FILE, 'r') as f:
            return f.read().strip()
    return EVIDENCE_FILE_DEFAULT

def lock_session():
    """Locks the current session file."""
    current_file = get_current_filename()
    if Path(current_file).exists():
        locked_name = f"{current_file}.LOCKED"
        try:
            Path(current_file).rename(locked_name)
            print(f"[TAMPER] Session Locked: {locked_name}")
        except Exception as e:
            print(f"[TAMPER] Lock Failed: {e}")
            
    if Path(SESSION_POINTER_FILE).exists():
        os.remove(SESSION_POINTER_FILE)
...
def load_chain(filename=None):
    """Load evidence chain from file"""
    target_file = filename or get_current_filename()
        ...
        try:
            with open(target_file, 'r') as f:
                ...
                if content:
                    ...
                    if "last_hash" not in data: data["last_hash"] = "0"
                    return data
        except (json.JSONDecodeError, IOError):
            pass  # File is corrupted, start fresh
    return {"chain": [], "last_hash": "0"}

def save_chain(chain, filename=None):
    """Save evidence chain to file"""
    target_file = filename or get_current_filename()
    with open(target_file, 'w') as f:
        json.dump(chain, f, indent=2)

def verify_chain(filename=None):
